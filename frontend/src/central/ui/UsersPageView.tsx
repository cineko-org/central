...
export function UsersPageView(props: UsersPageViewProps) {
  return (
    <Stack gap={40}>
      ...
      {props.issued ? (
        <Alert color="green" title={`${props.issued.user.displayName} PIN`} withCloseButton onClose={props.onDismissIssue}>
          ...
          <Table.Tbody>
            {props.users.map((item) => (
              <Table.Tr key={item.user.id}>
                <Table.Td><Text fw={600}>{item.user.displayName}</Text><Text size="xs" c="dimmed">{item.user.id}</Text></Table.Td>
                ...
                <Table.Td><Group justify="flex-end" gap="xs"><Button variant="subtle" color="gray" loading={props.loading} onClick={() => props.onRotate(item.user.id)}>{item.pinActive ? '재발급' : '발급'}</Button><Button variant="subtle" color="red" onClick={() => props.onDeleteRequest(item)}>사용자 제거</Button></Group></Table.Td>
              </Table.Tr>
            ))}
