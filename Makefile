...
contract-check:
	grep -Eq '^# github.com/cineko-org/contracts/v3 v3.4.1( => ../contracts)?$$' vendor/modules.txt

contract-release-check:
	@! grep -Eq '^[[:space:]]*replace([[:space:]]|\()' go.mod
	@grep -Eq '^[[:space:]]*github.com/cineko-org/contracts/v3 v3.4.1$$' go.mod
	@grep -Eq '^# github.com/cineko-org/contracts/v3 v3.4.1$$' vendor/modules.txt
	@grep -Eq '^github.com/cineko-org/contracts/v3 v3.4.1 h1:' go.sum
