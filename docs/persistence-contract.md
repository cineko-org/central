...
| Catalog snapshot | Require an online capable Client Probe for direct Client writes. In a serializable transaction, lock `catalog_state`, then additively upsert provider/theaters/movies/auditoriums/showtimes by canonical identity and content hash, including the provider-owned `schedule_date` separately from civil `starts_at`; omission never deletes/deactivates; a newly discovered auditorium without a current layout is queued for seat-map collection only when a future showtime exists, otherwise it remains idle until an explicit request | content-identical replay does not increment generation; HTTP request key is presence-only; partial writes never clear a pending full refresh; extended-clock showtimes preserve their original provider date |
| Seat-map version | A completed Central assignment is validated against its auditorium, then a serializable transaction inserts immutable `seat_map_versions` metadata and bulk-copies seats, seat features, source classes, zones, and blocks into normalized child tables. The same transaction sets `auditoriums.current_seat_map_version_id`, removes the auditorium's non-idle collection state, and increments catalog generation on change | `(auditorium_id, layout_hash)` is unique; child rows are version-scoped and cascade only with their immutable version; Client has no direct seat-map write endpoint |
| Seat-map collection request | Upsert the auditorium's `seat_map_collection_states` row. Central owns `queued`, `collecting`, `waiting_for_showtime`, `retry_scheduled`, and `blocked`; it records the trigger, exact showtime hint, assignment, retry timestamps, failure count, and reason. The mutation emits `cineko_seat_map_collection` in the same transaction | repeated requests coalesce; a Client request does not clear `blocked`; only a new operator, catalog identity, or layout-change trigger may requeue it, and never by creating a second collecting assignment |
| Exact live-seat snapshot | Validate the completed exact-showtime assignment, normalize the adjacent availability state into `seat_availability_snapshots` and ordered seat rows, evaluate every matching preset, and enqueue any false-to-true wake in the same result transaction. When the result carries a layout, persist both layout and availability atomically and finish the collection state without a follow-up seat-map assignment | unchanged content hashes are not stored twice; stale snapshots cannot reverse the match edge; a missing or changed layout queues one durable collection state rather than a timestamp-only request |
...
| Reconciler cycle | One advisory-lock transaction expires leases, processes results, advances durable seat-map collection states, schedules exact availability and user-demand discovery ahead of catalog/seat-map maintenance, prunes retained rows, and advances policy state. Collection selection requires a due state, an exact showtime hint when known, an available Probe slot, and backlog-aware priority | only the elected leader mutates a cycle; terminal collection outcomes update the state instead of recreating assignments every tick; same-transaction `NOTIFY` wakes the scheduler; unique active-policy, exact-showtime, and assignment indexes prevent duplicate active work |
