...
func TestBootstrapCapabilitiesAreCanonicalAndExactlyBound(t *testing.T) {
	...
	for _, invalid := range [][]string{nil, {}, {" "}, {"unsupported.v1"}} {
		if normalized, valid := normalizeCapabilities(invalid); valid || normalized != nil {
			t.Fatalf("normalizeCapabilities(%q) = %q, %t", invalid, normalized, valid)
		}
	}
}
