...
func TestSchemaContractMatchesMigrationHistory(t *testing.T) {
	...
	currentSchema := map[string][]string{
		...
		"showtime_observations":            {"assignment_id", "run_id", "target_date", "source_key", "theater_id", "auditorium_name", "screen_types", "movie_title", "poster_url", "starts_at", "ends_at", "available_seats", "capacity", "sold_out", "observed_at", "auditorium_id"},
		...
		"movies":                           {"id", "provider_id", "source_key", "title", "poster_url", "active", "content_hash", "first_seen_at", "last_seen_at", "updated_at"},
